...
class VideoLibrary:
    ...
    def get_random_video_id(self)->Optional[str]:
        try:
            return random.choice([video.video_id for video in self.get_all_videos()])
        except IndexError:
            return None
        
    def search_videos(self,search_term:str):
        search_term=search_term.lower()
        return[v for v in self.get_all_videos() if search_term in v.title.lower()]
    
    def get_videos_with_tag(self,tag:str):
        return [v for v in self.get_all_videos() if tag in v.tags]
